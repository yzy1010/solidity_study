...
import {ReentrancyGuard} from "@chainlink/contracts/node_modules/@openzeppelin/contracts/security/ReentrancyGuard.sol";
...
import {ReentrancyGuard} from "@chainlink/contracts/src/v0.7/vendor/ReentrancyGuard.sol";
import {Ownable} from "@openzeppelin/contracts-v0.7/access/Ownable.sol";
import {ReentrancyGuard} from "@openzeppelin/contracts-v0.7/utils/ReentrancyGuard.sol";
...
/**
 * @title AuctionMarket
 * @dev NFT Auction Market with Chainlink price feeds and upgradeable support
 */
contract AuctionMarket is Ownable, ReentrancyGuard {
    ...
    function placeBid(uint256 _auctionId) external payable nonReentrant {
        ...
        emit HighestBidIncreased(_auctionId, msg.sender, bidAmount, auction.highestBid);
    ...
    function claimRefund(uint256 _auctionId) external nonReentrant {
        ...
        emit RefundClaimed(msg.sender, refundAmount, auction.isERC20Payment);
    }
    ...
    /**
     * @dev Gets current USD prices
     */
    function getCurrentPrices() external view returns (uint256 ethUsd, uint256 erc20Usd) {
        ethUsd = uint256(ethUsdPriceFeed.getLatestPrice());
        erc20Usd = uint256(erc20UsdPriceFeed.getLatestPrice());
    }
